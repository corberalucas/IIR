...
#define     IN_SOUND_DEVICE      "plughw:0,0"	// Use for PS EYE input
...
// *************************
    ...
    char *outputBuffer = NULL;	// Output buffer for driver to read from
       ...
       //numero maximo de muestra de entrada
#define MAX_INPUT_LEN   8192
// numero maximo de coeficientes
#define MAX_FLT_LEN     100
// tamaño del buffer de entrada luego de aplicar el filtro
#define BUFFER_LEN      (MAX_FLT_LEN - 1 + MAX_INPUT_LEN)


//modificar para hacer el filtro de mayor orden
#define FILTER_LEN_MA   8

//    char insamp [BUFFER_LEN];
//	char *insamp = NULL;
	short *state = NULL;
    	int j;
...
// ******************************************************
    ...
    DBG( "Reservando buffer de entrada de tamanaño %d en la direccion %p\n", blksize, inputBuffer );
    ...
    DBG( "Reservando buffer de salida de tamanaño %d en la direccion %p\n", blksize, outputBuffer );

    // Record that the output buffer was allocated in initialization bitmask
    initMask |= OUTPUT_BUFFER_ALLOCATED;

    /*
	if( ( insamp = malloc( blksize+MAX_FLT_LEN - 1 ) ) == NULL )
    {
        ERR( "Fallo la memoria para el buffer del filtro FIR(%d)\n", blksize );
        status = AUDIO_THREAD_FAILURE;
        goto  cleanup ;
    }
*/
//    DBG( "Memoria de entrada para el filtro FIR %d en la direccion %p\n", blksize+MAX_FLT_LEN - 1, insamp );
	
	if( ( state = malloc( blksize/2 ) ) == NULL )
    {
        ERR( "Fallo la memoria para el buffer del filtro FIR(%d)\n", blksize );
        status = AUDIO_THREAD_FAILURE;
        goto  cleanup ;
    ...
    DBG( "Memoria de salida para el filtro FIR %d en la direccion %p\n", blksize/2, state );
...
// **************************************************
    ...
    memset(state,0, blksize/2);		// Clear the buffer
   // for(j=0;j<blksize/2;j++)
     // state[j]=0x0001;
    
	memset(outputBuffer, 0, blksize);		// Clear the buffer

	//pongo insamp todo a 0	
//	memset(insamp, 0, blksize+MAX_FLT_LEN - 1);

    printf( "Starting DSP..." );
    t_start = get_timestamp();

    // llamada tonta a la funcion para obtener el tiempo
    audio_process((short *)outputBuffer, (short *)outputBuffer, blksize/2,(short *)state);
    ...
    int i;
    memset(outputBuffer, 0, blksize);		// Clear the buffer
	//memset(outdsplib, 0, blksize/2);		// Clear the buffer
	//memset(insamp, 0, blksize+MAX_FLT_LEN - 1);
...
//
	...
	audio_process((short *)outputBuffer, (short *)inputBuffer, blksize/2,(short *)state);
	t_proc = get_timestamp();
...
//	memmove( &insamp[0], &insamp[blksize/2],(FILTER_LEN_MA - 1)* sizeof(short) );
